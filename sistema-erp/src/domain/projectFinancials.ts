...
export async function recalculateProjectFinancials(
  ...
  const base = budgetItems.reduce((sum, item) => sum.plus(item.originalAmount), new Prisma.Decimal(0));
  ...
  const spent = purchaseOrders
    ...
    .plus(subcontractorCertificates.reduce((sum, certificate) => sum.plus(certificate.amount), new Prisma.Decimal(0)));
  const real = base.plus(adendas._sum.monto_total ?? 0);
